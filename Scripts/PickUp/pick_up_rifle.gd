extends Area2D#pick_up_rifle

const weapon_id=876476543;
@export var weapon_scene = preload("res://Scenes/Weapons/rifle.tscn")  # The specific weapon scene
	...
	if player_in_area and Input.is_action_just_pressed("pick_up"):  # Wait for "E" key press to pick up
		...
		if player:
			...
			if weapon_manager:
				...
				weapon_manager.add_weapon_by_id(weapon_id, weapon_scene)  # Add the new weapon to the player's weapon manager
